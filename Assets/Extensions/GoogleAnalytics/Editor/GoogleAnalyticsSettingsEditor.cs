...
public class GoogleAnalyticsSettingsEditor : Editor {
	...
	GUIContent exTrackingLabel = new GUIContent("Exceptions [?]:", "Application exceptions reports will be sent automaticaly ");
	GUIContent systemInfoTrackingLabel = new GUIContent("SystemInfo [?]:", "System info will be automaticaly sumbited on first launch ");
	GUIContent quitTrackingLabel = new GUIContent("App Quit [?]:", "Automaticaly track app quit ");
	GUIContent pauseTrackingLabel = new GUIContent("App [?]:", "Automaticaly track when app goes backround and start / end seesion on this event ");
	...
	GUIContent UseHttpsLabel = new GUIContent("Use Https [?]", "Enable yo send data over SSL");
	GUIContent StringEscape  = new GUIContent("String Escape [?]", "Enable All strings  escaping using WWW.EscapeURL, Escaping of strings is safer, but adds additional RAM consumation");
		...
		if(GoogleAnalyticsSettings.Instance.showPlatfroms) {
			...
			if (settings.accounts.Count == 0) {
				...
				foreach(RuntimePlatform platfrom in (RuntimePlatform[]) System.Enum.GetValues(typeof(RuntimePlatform)) ) {
					...
					int index = GoogleAnalyticsSettings.Instance.GetProfileIndexForPlatfrom(platfrom, true);
					...
					index = GoogleAnalyticsSettings.Instance.GetProfileIndexForPlatfrom(platfrom, false);
					index = EditorGUILayout.Popup(index, GoogleAnalyticsSettings.Instance.GetProfileNames());
	...
	private void GeneralOptions() {
		...
		if(settings.UsePlayerSettingsForAppInfo) {
			...
			settings.AppName = PlayerSettings.productName;
	...
	private void ButtonsGUI() {
		...
		if(GUILayout.Button("Refresh Client Id",  GUILayout.Width(120))) {
			PlayerPrefs.DeleteKey(GoogleAnalytics.GOOGLE_ALANALYTICS_CLIENTID_PREF_KEY);
		}
